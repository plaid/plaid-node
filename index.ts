...
class Client {
  ...
  constructor(config: Config) {
    ...
    this.openAPIClient = new PlaidApi(
      {
        baseOptions: {
          headers: {
  ...
  getAssetReportPdf(
    ...
    return plaidRequest(
      this.openAPIClient.assetReportPDFGet(
        ...
        { json: true, responseType: 'arraybuffer' },
      ),
      cb,
    );
  }
  ...
  getLinkToken(
    link_token: string,
    cb?: Callback<LinkTokenGetResponse>,
  ): Promise<LinkTokenGetResponse> {
    return plaidRequest(
      this.openAPIClient.linkTokenGet({
        client_id: this.client_id,
        link_token,
        secret: this.secret,
      }),
...
const rejectWithPlaidError = (data: axios.AxiosError): PlaidError => {
  const error = data.response?.data;

  if (typeof error === 'object' && error !== null) {
    error.status_code = data.response?.status;
...
const wrapPromise = (caller: Promise<axios.AxiosResponse>) => {
...
const callbackOptions = (request: any, cb: Callback<axios.AxiosResponse>) => {
  return request
    .then((resp: axios.AxiosResponse) => {
      cb(null, resp.data);
    })
    .catch((err: axios.AxiosError, _: any) => {
      cb(rejectWithPlaidError(err), null);
    });
};

const plaidRequest = (
  caller: Promise<axios.AxiosResponse>,
  cb?: Callback<any>,
) => {
  if (cb) {
